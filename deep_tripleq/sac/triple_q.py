...
def sac(env_fn, actor_fn=mlp_actor, critic_fn=mlp_critic, var_fn=mlp_var, ac_kwargs=dict(), seed=0,
    ...
    x_ph, a_ph, x2_ph, r_ph, d_ph, c_ph, queue = placeholders(obs_dim, act_dim, obs_dim, None, None, None, None)
    #eta,queue = placeholders(None,None)
    #print(eta,queue)
    #queue = 0
    ...
    min_q_cpi = tf.minimum(qc1_pi, qc2_pi)
    min_q_cpi_targ = tf.minimum(qc1_pi_targ, qc2_pi_targ)

    q_backup = tf.stop_gradient(r_ph + gamma*(1-d_ph)*(min_q_pi_targ - alpha * logp_pi2))
    qc_backup = tf.stop_gradient(c_ph + gamma*(1-d_ph)*(min_q_cpi_targ - alpha * logp_pi2))

    pi_loss = tf.reduce_mean(alpha * logp_pi - min_q_pi - (queue[0] / eta) * min_q_cpi )
    #pi_loss = tf.reduce_mean(alpha * logp_pi - min_q_pi -  min_q_cpi )
    ...
    for t in range(total_steps // num_procs()):
        ...
        if d or (ep_len == max_ep_len):
            ...
            epcosts.append(ep_cost)
            if lll % 20 == 0:
                #queue_ = max(queue_ + (max_ep_len - cost_lim) + 2 - ep_cost, 0)
                queue_ = max(queue_ + (max_ep_len - cost_lim) + 2 - np.mean(epcosts), 0)
                print(f"virtual queue: {queue_}, cost_lim: {cost_lim}, ep_cost :{ep_cost}")
                epcosts = []
                lll = 0
            o, r, d, ep_ret, ep_cost, ep_len, ep_goals = env.reset(), 0, False, 0, 0, 0, 0
        ...
        if t > 0 and t % update_freq == 0:
            ...
            for j in range(update_freq):
                ...
                feed_dict = {x_ph: batch['obs1'],
                             ...
                             c_ph: batch['costs'],
                             d_ph: batch['done'],
                             queue: np.array([queue_] * local_batch_size),
                            }
